  ...
  if (is.null(model)) { # noise
    ...
    switch(param,
      "sigma" = if (stationary) {
        ...
        paste0("theta_sigma = ", val)
      },
      "sigma_nig" = if (stationary) {
        paste0("sigma_nig = ", val)
      } else {
        paste0("theta_sigma_nig = ", val)
      },
      "sigma_normal" = if (stationary) {
        paste0("sigma_normal = ", val)
      } else {
        paste0("theta_sigma_normal = ", val)
      },
      "mu" = if (stationary) {
        paste0("mu = ", val)
      } else {
...
validate_rep_or_group <- function(replicate, data) {
  ...
  if (inherits(replicate, "formula")) {
    ...
    stopifnot(
      "Allow 1 variable (column in data) as replicate. i.g. replicate=~id" = length(replicate) == 2 && length(replicate[[2]]) == 1
    )
  ...
  stopifnot(
    "Please make sure the length of replicate/group is equal to the number of rows of data" = nrow(data) == length(replicate)
  )
...
ngme_build_A <- function(model, mesh, map, operator, group, group_levels = NULL) {
  ...
  if (model %in% c("tp", "spacetime")) {
    ...
    stopifnot(
      "Now only support first to be 1d model" = inherits(mesh_t, "inla.mesh.1d")
    )
    ...
    if (model == "tp" && operator$second$model %in% c("bv", "bv_matern", "bv2")) {
      # for tp-bv model
      # check if group is valid
...
contain_bv_model <- function(ngme) {
  any(sapply(ngme$replicates[[1]]$models, function(model) {
    model$model %in% c("bv", "bv2", "bv_matern")
  })) ||
    any(sapply(ngme$replicates[[1]]$models, function(model) {
      model$model %in% c("tp") && model$operator$second$model %in% c("bv", "bv2", "bv_matern")
    }))
}
