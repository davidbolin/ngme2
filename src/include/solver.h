...
class solver {
  ...
  public:
    ...
    virtual inline Eigen::VectorXd Qinv_diag(){Eigen::VectorXd a; a.setZero(1); return a;}
    virtual Eigen::VectorXd rMVN( Eigen::VectorXd&, Eigen::VectorXd &) = 0;
    virtual SparseMatrix<double,0,int> return_Qinv(){SparseMatrix<double,0,int> a; std::cout << "return_Qinv not implimented for MatrixXd\n"; return a;}
};
...
class iterative_solver : public virtual solver
{
  ...
  public:
    ...
    Eigen::VectorXd rMVN( Eigen::VectorXd& mu , Eigen::VectorXd & z){std::cout << "rMVN not implimented\n"; Eigen::VectorXd X; return X;}
...
class cholesky_solver : public virtual solver
{
  ...
  public:
    ...
    double trace(Eigen::SparseMatrix<double,0,int>&);
    double trace2(SparseMatrix<double,0,int>&, SparseMatrix<double,0,int>&);
    inline double logdet(){set_ld(); return ld;}
    VectorXd Qinv_diag();
...
class lu_solver : public virtual solver
{
  ...
  public:
     ...
     double trace2(SparseMatrix<double,0,int>&, SparseMatrix<double,0,int>&);
    double logdet();
    Eigen::VectorXd rMVN( Eigen::VectorXd&, Eigen::VectorXd &){std::cout << "lu_solver:rMVN not implimented\n"; throw;};
};
...
class lu_sparse_solver : public virtual solver
{
  ...
  public:
    ...
    double logdet();
    Eigen::VectorXd rMVN( Eigen::VectorXd&, Eigen::VectorXd &){std::cout << "lu_sparse_solver:rMVN not implimented\n"; throw;};
};
