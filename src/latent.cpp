...
void Latent::update_derivatives(
    ...
    bool reset = (!deriv_cache.initialized) || (deriv_cache.rao_blackwell != rao_blackwell);
    if (reset) {
    ...
    if (need_grad_theta_K) compute_theta_K(true, rao_blackwell);
    if (need_grad_theta_mu) compute_theta_mu(true, rao_blackwell);
    if (need_grad_theta_sigma) compute_theta_sigma(true, rao_blackwell);
    if (need_grad_theta_nu) compute_theta_nu(true);
}
...
void Latent::compute_theta_sigma(bool need_grad, bool rao_blackwell) {
    if (!need_grad || deriv_cache.grad_sigma_ready) return;

    VectorXd grad = VectorXd::Zero(n_theta_sigma);
    bool all_fixed = std::all_of(fix_theta_sigma_vec.begin(), fix_theta_sigma_vec.end(), [](bool b) { return b; });
    if (!all_fixed) {
        VectorXd WW = (rao_blackwell) ? cond_W : W;
        VectorXd SV = sigma.array().pow(2).matrix().cwiseProduct(V);
...
void Latent::compute_theta_nu(bool need_grad) {
    if (!need_grad || deriv_cache.grad_nu_ready) return;
...
void Latent::update_each_iter() {
    ...
    t_noise_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - t_noise_start).count();
    ...
    if (!fix_flag[latent_fix_theta_K] && !zero_trace) {
        ...
        if (tv.size() == n_theta_K) {
            for (int i=0; i<n_theta_K; ++i) trace[i] = tv(i);
        }
        t_trace_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - t_trace_start).count();
    }
...
void Latent::compute_hessian_blocks(bool rao_blackwell) {
    (void)rao_blackwell;
    // Initialize blocks
    if (n_theta_K > 0) { hess_cache.H_K = MatrixXd::Zero(n_theta_K, n_theta_K); }
    if (n_theta_mu > 0) { hess_cache.H_mu = MatrixXd::Zero(n_theta_mu, n_theta_mu); }
    if (n_theta_sigma > 0) { hess_cache.H_sigma = MatrixXd::Zero(n_theta_sigma, n_theta_sigma); }
    if (n_theta_nu > 0) { hess_cache.H_nu = MatrixXd::Zero(n_theta_nu, n_theta_nu); }
    if (n_theta_K > 0 && n_theta_mu > 0) { hess_cache.H_K_mu = MatrixXd::Zero(n_theta_K, n_theta_mu); }
    if (n_theta_K > 0 && n_theta_sigma > 0) { hess_cache.H_K_sigma = MatrixXd::Zero(n_theta_K, n_theta_sigma); }
    ...
    if (n_theta_K > 0 && n_theta_mu > 0) {
        ...
        VectorXd Dinv = (sigma.array().square().matrix().cwiseProduct(V)).cwiseInverse();
        VectorXd diagVH = (V - h).cwiseProduct(Dinv); // elementwise (V-h)/(sigma^2 V)
        for (int j=0; j<n_theta_K; ++j) {
    ...
    if (n_theta_mu > 0 && n_theta_sigma > 0) {
        ...
        // The provided form is d^2/d(theta_sigma d theta_mu^T) = -2 B_sigma^T diag(...) B_mu
        // Our cache stores mu x sigma; so take transpose
        ...
        if (noise_type[0] != "normal") {
            if (n_noise == 1) {
                if (noise_type[0] == "normal_nig") {
                    ...
                    VectorXd V_half    = V.segment(0, V.size()/2);
                    VectorXd h_half    = h.segment(0, h.size()/2);
                    hess_cache.H_nu = NoiseUtil::hess_theta_nu(noise_type[0], B_nu_half, nu_half, V_half, h_half, single_V);
                } else {
                    hess_cache.H_nu = NoiseUtil::hess_theta_nu(noise_type[0], B_nu, nu, V, h, single_V);
                }
            } else {
    ...
    if (n_theta_sigma > 0) hess_cache.H_sigma.diagonal().array() += 1e-6;
    if (n_theta_nu > 0) hess_cache.H_nu.diagonal().array() += 1e-6;
