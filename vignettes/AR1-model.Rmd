...
print(data.frame(
  Truth = c(-1, 2, 0.5),
  Estimates = as.numeric(all_params$data$fixed_effects),
  row.names = names(all_params$data$fixed_effects)
))

# Measurement Noise
print(data.frame(Truth = 2.0, Estimate = all_params$data$sigma, row.names = "sigma"))
cat("\n")

# AR(1) Process Parameters:
print(data.frame(
  Truth = c(rho = 0.5, mu = -3.0, sigma = 4.0, nu = 0.4),
  Estimates = unlist(all_params$my_ar)
))
```

### Model Validation

Finally, let us validate our model by comparing the estimated noise distribution with the true distribution:

```{r model-validation, fig.height=5, fig.width=7}
# Extract the estimated AR(1) model and create validation plot
ar1_estimated <- ngme_result(ngme_out, "my_ar")

# Distribution Validation: Fitted vs. True NIG Noise Model
# plot(ar1_estimated$noise, noise_nig(mu = -3, sigma = 4, nu = 0.4))
```
