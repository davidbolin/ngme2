...
### Approach 1: Using the ~/.R/Makevars file (recommended)
...
### Approach 2: Set up LLVM Clang as your system compiler

This approach involves installing LLVM/Clang and making it your default system compiler. This affects not just R but all compilation on your system.

1. First, install LLVM using Homebrew:

```
brew install llvm
```

2. Add the LLVM binary directory to your PATH by adding this line to your shell profile (e.g., ~/.zshrc or ~/.bash_profile):

```
echo 'export PATH="/opt/homebrew/opt/llvm/bin:$PATH"' >> ~/.zshrc
```

3. Reload your shell profile or restart your terminal session:

```
source ~/.zshrc
```

4. Verify that LLVM's clang is now your default compiler:
...
5. Test that OpenMP is supported:
...
clang -fopenmp test_omp.c -o test_omp
./test_omp
```

If OpenMP is working correctly, you should see output from multiple threads.

6. Configure R to use your system compiler by creating a `~/.R/Makevars` file:
...
After completing these steps, reinstall the `ngme2` package to ensure it's built with OpenMP support.

Both approaches will enable parallel processing features in the `ngme2` package. Approach 1 is recommended for most users as it only affects R package compilation without changing your system's default compiler.

## Linux

Most Linux distributions come with GCC/G++ compilers that already support OpenMP. Here's how to ensure your system is ready for `ngme2` with OpenMP support:
