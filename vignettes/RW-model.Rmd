...
p3 <- ggplot(nig_data, aes(x = time, y = value, color = model)) +
  ...
  labs(
    ...
    subtitle = "RW1 vs RW2 with NIG(mu = -3, sigma = 4/1.5, nu = 0.4) - Independent Scales",
    x = "Time",
    y = "Value",
    color = "Model"
  ) +
...
# Keep the original observation order. Sorting by Y makes the observed line
# monotone by construction and can create a misleading visual mismatch with the
...
plot(obs_idx, Y,
  ...
  ylim = range(c(Y, y_quantiles))
)

# Add uncertainty bands
polygon(c(obs_idx, rev(obs_idx)),
  c(y_quantiles[1, ], rev(y_quantiles[5, ])),
  col = rgb(0.64, 0.23, 0.45, 0.2), border = NA
)
polygon(c(obs_idx, rev(obs_idx)),
  c(y_quantiles[2, ], rev(y_quantiles[4, ])),
  col = rgb(0.64, 0.23, 0.45, 0.3), border = NA
)

lines(obs_idx, y_quantiles[3, ], col = rgb(0.64, 0.23, 0.45, 0.8), lwd = 1)
lines(obs_idx, Y, col = "#2E86AB", lwd = 1)
