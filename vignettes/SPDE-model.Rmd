...
$$\left(\kappa^2 - \Delta\right)^{\alpha/2} u = \mathcal{W} \quad \text{on} \quad \mathbb{R}^d,$$
...
traceplot(out, "data", hline = sigma_noise)
```
...
params_data <- ngme_result(out, "data")
cat("Fixed effect (salinity):", round(as.numeric(params_data$beta), 3), "\n")
...
ggplot(kappa_df, aes(x = x, y = y, color = kappa)) +
  ...
  coord_fixed() +
  labs(
    title = "True Spatial Variation in kappa",
    subtitle = "log(kappa) = 0.5 + 1.5x - 0.8x^2",
    x = "Longitude",
    y = "Latitude",
    color = "kappa value"
  ) +
