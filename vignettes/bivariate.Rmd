...
### Ex1. Fixed effects + Bivariate model (AR1) + Correlated measurement noise
...
traceplot(
  out, "bv",
  hline=c(pi/8, 0.5, 0.6, -0.4, -2, 2, 1, 2, 1, 0.5)
)
...
### Ex2. Fixed effects + Bivariate model (Matérn) + Correlated measurement noise
...
true_model <- f(
  ...
  model = "bv",
  sub_models = list(
    W1 = list(model = "matern", theta_K = log(4)),
    W2 = list(model = "matern", theta_K = log(6))
  ),
  group = group,
...
Y <- as.numeric(true_model$A %*% W) + rnorm(n_obs, sd=0.2)
...
out <- ngme(
  Y ~ 0 + f(
    ...
    model = "bv",
    name = "bv",
    sub_models = list(W1="matern", W2="matern"),
...
traceplot(
  out, "bv",
  hline=c(pi/8, 0.5, 4, 6, -2, 2, 1, 2, 1, 0.5)
)
