...
# Choose model="bv2" if use Gaussian noise,
bv1 <- f(
  year,
  model = "bv2",
  theta = pi / 8, rho = 0.5,
...
Next, we consider a spatial bivariate Matérn model with correlated noise.
...
Let's run a more complex example using the bivariate model. We will generate some data and fit the bivariate model using the `ngme2` function. Then, we will perform cross-validation using the `cross_validation` function.

```{r}
cv1 <- cross_validation(
  bvar_cor,
  k = 5, N = 3, n_gibbs_samples = 50
)
cv1$mean.scores
cv1$sd.scores
```
