...
The model is:
$$ \mathbf{D(\theta, \rho)} diag(L_1, L_2) \mathbf{X}(s) = \mathcal{M},$$ where $\mathbf{D}$ is the dependence matrix, $\mathcal{M}$ is non-Gaussian noise, we will discuss the noise later.
...
\end{pmatrix},
$$
$\rho \in [-1, 1]$  controls the cross-correlation.
...
In bivariate models, we can have more control over the noise of the model. The noise can be classified into 4 category (See Bolin and Wallin, 2020, section 3.1 for details):
...
Next we see an less artificial example. We can use the bivaraite model to model 2 spatial fields.
