...
A_list <- replicate(n_each, Cov_same_idx, simplify = FALSE)
Cov_measurement <- Matrix::bdiag(A_list)
...
traceplot(bvar_ind, "bv", hline=c(3,1,1,0.6,-0.4))
traceplot(bvar_ind, hline=c(log(sd_1), log(sd_2), -3, 1.5))
...
out$replicates[[1]]$Y
out$replicates[[1]]$noise$index_corr

# comparing with simulated value
traceplot(
  out, "bv",
  hline=c(3, 2, 1, 0.6, -0.4)
)
traceplot(out1, hline=c(log(sd_1), log(sd_2), rho_e, -3, 1.5))
# traceplot(out, hline=c(log(sd_1), log(sd_2), -3, 1.5))
...
traceplot(
  out_cor,
  hline=c(log(sd_1), log(sd_2), rho_e)
)
```
