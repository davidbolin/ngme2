...
# The Parana section at the end of this vignette uses datasets (PRprec,
# PRborder) and helpers (spDists) that come from INLA. INLA is not on CRAN and
# is only Suggested here, and its fmesher dependency is version sensitive, so
# loading it can fail even where it is installed -- requireNamespace() returns
# FALSE in both cases. Each chunk down there carries the same probe in its
# `eval` option, which is what actually skips them; this copy only drives the
# note printed at the top of the section. The probe has to be repeated per
# chunk rather than set once via opts_chunk$set(): R CMD check tangles the
# vignette with knitr::purl(), and purl only evaluates chunk options that are
# self-contained, so a variable defined in an earlier chunk would break it.
have_inla <- isTRUE(tryCatch(
  suppressWarnings(requireNamespace("INLA", quietly = TRUE)),
...
`ngme2` monitors convergence with two diagnostics, both enabled by default. They run at each checkpoint (every `iters_per_check` iterations, default `iterations / n_batch`) and only begin after `n_min_batch` checkpoints have accumulated. All of this needs more than one chain: with `n_parallel_chain = 1` no convergence checking runs at all, and the optimizer always uses the full `iterations` budget.

- **Gelman–Rubin R̂ (`R_hat_conv_check`, `max_R_hat`)**  
  Compares between- and within-chain variance for each parameter; passes if `R_hat <= max_R_hat` (default 1.1). A degenerate within-chain variance (a frozen chain, say) is reported as failing rather than as `NaN`.

- **Trend diagnostic (`trend_std_conv_check`)**  
  Fits a weighted linear trend to the last `n_slope_check` checkpoints (default 3) and passes when the slope is not distinguishable from zero, `|slope| / se(slope) <= trend_lim` (default 2). This is scale-free: it depends neither on the parameter's units nor on `n_batch`. Setting `trend_use_tstat = FALSE` recovers the absolute test `|slope| <= trend_lim`. The check cannot pass before the slope window is full.  
  Its relative-standard-deviation part, `sqrt(var) / |mean| <= std_lim` (default 0.01), is **off by default** (`use_std_check = FALSE`). The coefficient of variation conflates "the chains disagree" with "this parameter is imprecisely determined", so a weakly identified parameter such as `nu` could never pass it, and R̂ already measures between- vs within-chain disagreement in a scale-free way.

Combination rule:
- A parameter is marked converged only if **every** enabled diagnostic passes for it, so a single lenient check cannot declare convergence on its own. Drop one from the requirement with `R_hat_conv_check = FALSE` or `trend_std_conv_check = FALSE`.  
- Overall convergence is reached when all parameters are converged, and the criteria must hold on `n_conv_batch` consecutive checkpoints (default 1), since a single passing checkpoint is weak evidence.
...
ctrl <- control_opt(
  ...
  n_conv_batch = 1,           # consecutive passing checkpoints required
  R_hat_conv_check = TRUE,
  max_R_hat = 1.1,
  trend_std_conv_check = TRUE,
  trend_use_tstat = TRUE,     # test |slope| / se(slope), not |slope|
  trend_lim = 2,
  use_std_check = FALSE,      # also require sqrt(var)/|mean| <= std_lim
  std_lim = 0.01,
...
Recommendations:
- Use at least two chains. With one chain there is no convergence checking, no early stopping, and no `warn_no_convergence` warning -- the run simply uses the whole budget.
- If checks trigger too early, increase `n_min_batch` or `n_conv_batch`; if they never trigger, loosen `trend_lim` and `max_R_hat`.
...
```{r parana-note, echo = FALSE, eval = TRUE, results = "asis"}
if (!have_inla) {
  cat("> **Note:** INLA is not available, so the code in this section is",
...
```{r read-parana, eval = requireNamespace("INLA", quietly = TRUE)}
library(INLA)
data(PRprec)
data(PRborder)
...
```{r plot-data, message=FALSE,fig.width=6,fig.height=6,fig.align='center', echo=FALSE, fig.cap="Mean of the rainfall in Octobor 2012 in Paraná", eval = requireNamespace("INLA", quietly = TRUE)}
ggplot() +
  geom_point(aes(
...
```{r fit-parana, cache=TRUE, eval = requireNamespace("INLA", quietly = TRUE)}
...
```{r traceplot-parana, eval = requireNamespace("INLA", quietly = TRUE)}
# traceplots
## fixed effects and measurement error
traceplot(parana_fit, "data")
...
```{r est-result, echo=FALSE, eval = requireNamespace("INLA", quietly = TRUE)}
...
```{r predict, cache=TRUE, eval = requireNamespace("INLA", quietly = TRUE)}
nxy <- c(150, 100)
...
The following code shows how to do it. For the details of the cross-validation, see the [cross-validation vignette](articles/cross-validation.html).

```{r fit-gauss, cache=TRUE, eval = requireNamespace("INLA", quietly = TRUE)}
...
```{r cv-gauss, cache=TRUE, eval = requireNamespace("INLA", quietly = TRUE)}
cv <- cross_validation(
  list(
