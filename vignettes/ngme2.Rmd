...
out <- ngme(
  formula = Y ~ 1 +
    f(inla.group(seaDist), name="rw", model = "rw1", noise = noise_normal()) +
    f(model = matern_spde, name="spde", noise = noise_normal()),
  data = list(
    Y = Y
  ),
  family = noise_nig(),
...
lp <- predict(out, loc=list(seaDist.prd, coord.prd))[["mean"]]
