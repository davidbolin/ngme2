...
Optimizer options can be set through `control_opt`. The supported optimizers are listed in `ngme_optimizers`. 

```{r}
ngme_optimizers()
# For details about optimizer options, see e.g. help(adam)
```
...
The optimization process is controlled by the `control_opt` argument.

Different stochastic optimization algorithms can be used for the optimization process.
See `?ngme_optimizers` for more details.
