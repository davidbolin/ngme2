...
\begin{subequations} \label{eq:framework}
\begin{align}
\text{Data:} \quad & \mathbf{Y} = \mathbf{A}\mathbf{W} + \mathbf{X}\boldsymbol{\beta} + \boldsymbol{\epsilon}^{(1)} \label{eq:data}\\
\text{Process:} \quad & \mathbf{K}(\boldsymbol{\theta})\mathbf{W} = \boldsymbol{\epsilon}^{(2)} \label{eq:process}\\
\text{Noise:} \quad & \boldsymbol{\epsilon}^{(i)} | \mathbf{V}^{(i)} \sim \mathcal{N}(\mathbf{m}^{(i)}, \boldsymbol{\Sigma}^{(i)}), \quad i = 1, 2 \label{eq:noise} \\
...
\text{Distribution:} \quad & \mathbf{V}_j^{(i)} \overset{\text{iid}}{\sim} \text{GIG}(\mathbf{p}_j^{(i)}(\boldsymbol{\theta}), \mathbf{a}_j^{(i)}(\boldsymbol{\theta}), \mathbf{b}_j^{(i)}(\boldsymbol{\theta})) \label{eq:distribution} \quad i = 1, 2, \quad j = 1, \ldots, n_i
\end{align}
\end{subequations}

The data model \eqref{eq:data} links observations $\mathbf{Y} \in \mathbb{R}^{n_1}$ to the latent process $\mathbf{W} \in \mathbb{R}^{n_2}$ through the observation matrix $\mathbf{A} \in \mathbb{R}^{n_1 \times n_2}$, adds fixed effects $\mathbf{X}\boldsymbol{\beta}$, and allows non-Gaussian measurement noise $\boldsymbol{\epsilon}^{(1)} \in \mathbb{R}^{n_1}$.

The process model \eqref{eq:process} specifies the core dependency structure of $\mathbf{W}$ via the operator matrix $\mathbf{K}(\boldsymbol{\theta}) \in \mathbb{R}^{n_2 \times n_2}$, which is driven by potentially non-Gaussian noise $\boldsymbol{\epsilon}^{(2)} \in \mathbb{R}^{n_2}$. This operator-based formulation unifies a wide spectrum of model structures, from simple random effects to complex spatio-temporal patterns.

Here $\odot$ denotes element-wise multiplication. The noise model \eqref{eq:noise}-\eqref{eq:distribution} defines $\boldsymbol{\epsilon}^{(1)}$ and $\boldsymbol{\epsilon}^{(2)}$ through normal mean-variance mixtures conditioned on the GIG mixing vectors $\mathbf{V}^{(1)} \in \mathbb{R}^{n_1}$ and $\mathbf{V}^{(2)} \in \mathbb{R}^{n_2}$. The parameter vector $\boldsymbol{\theta}$ collects all model parameters.
...
res_gauss_nig <- data.frame(
  intercept    = format(m_gauss_nig$replicates[[1]]$feff, digits = 3),
  noise_mu     = format(m_gauss_nig$replicates[[1]]$noise$theta_mu, digits = 3),
  noise_sigma  = format(exp(m_gauss_nig$replicates[[1]]$noise$theta_sigma), digits = 3),
  noise_nu     = format(exp(m_gauss_nig$replicates[[1]]$noise$theta_nu), digits = 3),
  rw_sigma     = format(m_gauss_nig$replicates[[1]]$models[[1]]$noise$theta_sigma, digits = 3),
  ma_kappa     = format(exp(m_gauss_nig$replicates[[1]]$models[[2]]$theta_K), digits = 3),
  ma_sigma     = format(exp(m_gauss_nig$replicates[[1]]$models[[2]]$noise$theta_sigma), digits = 3)
)
knitr::kable(res_gauss_nig, caption = "Estimations for the model")
```

Similarily, we can fit some different models:

```{r fit-different-models, cache=TRUE}
m_gauss_gauss <- ngme(
  formula = Y ~ 1 +
    f(seaDist, name = "rw1", model = "rw1", noise = noise_normal()) +
    f(coords, model = "matern", mesh = prmesh, name = "spde", noise = noise_normal()),
