...
Q_matrix <- t(K_matrix) %*% solve(H, K_matrix)
...
p1 <- ggplot(gaussian_data, aes(x = time, y = value)) +
  ...
  annotate("text", x = max(time_index) * 0.9, y = 1, 
...
cat("Empirical mean:", round(mean(W_gaussian), 2), 
    "(theoretical: 0)\n")

theoretical_var <- sigma_sim^2 / (2 * theta_sim)
cat("Empirical variance:", round(var(W_gaussian), 2), 
...
p3 <- ggplot(nig_data, aes(x = time, y = value)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", 
             color = "#F18F01", linewidth = 0.8) +
  annotate("text", x = max(time_index) * 0.9, y = 2, 
           label = "mean = 0", color = "#F18F01", size = 4) +
  labs(
...
p_theta_comparison <- ggplot(ou_comparison_data, 
...
ngme_out <- ngme(
  Y ~ 0 + x1 + x2 + x3 + f(
    ...
    name = "my_ou",
    model = "ou",
    theta = theta_sim,  # Initial value for theta
    noise = noise_nig(),
...
p_holdout <- ggplot() +
  geom_line(data = viz_data, aes(x = time, y = value, color = type), 
            linewidth = 0.4, alpha = 0.7) +
  geom_ribbon(data = pred_data, 
              aes(x = time, ymin = lower, ymax = upper),
              fill = "#A23B72", alpha = 0.2) +
  geom_line(data = pred_data, aes(x = time, y = pred),
            color = "#A23B72", linewidth = 0.6) +
  geom_point(data = pred_data, aes(x = time, y = pred),
...
for(i in 1:3) {
  ...
  p <- ggplot() +
    ...
    geom_point(data = gap_true_data, aes(x = time, y = value),
    ...
    geom_vline(xintercept = time_index[c(min(gap$indices), max(gap$indices))],
...
legend("topleft",
       ...
       col = c("#2E86AB", rgb(0.64, 0.23, 0.45, 0.8), "#2E86AB", 
...
legend("topleft", 
       ...
       col = c("#2E86AB", rgb(0.64, 0.23, 0.45, 0.8), 
...
text(c(0.7, 1.9), c(0.55, 1.0), 
     labels = c(paste(round(coverage_50*100, 1), "%"),
