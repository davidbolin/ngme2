...
# fit the model
ngme_out <- ngme(
  formula = Y ~ x1 + f(model="ar1", noise=ngme.noise.nig()),
  data = list(Y=Y),
...
Finally, we can extract the prediction from result using `$prediction`. 

```{r}
str(ngme_out$prediction)
```
...
lines(ngme_out$prediction$response[NA_index], col="green") #predictions

# Plot the results for all location
plot(W + feff, type="l", 
     main="prediction vs. true fields at all index", ylab="value")  # true fields
lines(ngme_out$est_output$latents[[1]]$W 
     + drop(X %*% ngme_out$est_output$fixed_effects), col="red")
# lines(ngme_out$prediction$response, col="red") #predictions
points(Y)
