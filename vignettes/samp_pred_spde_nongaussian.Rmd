...
We will take $\kappa=1$, $\sigma_\varepsilon=1$, $\sigma=1$, $\mu = 1$, $\mathcal{D}=[0,1]$ and $\eta=0.5$. We will also assume that we
...
temp = rnorm(n, mean = (-h_nig_1d + V)*mu, sd = sigma^2*sqrt(V))
W = solve(K, temp)
...
Recall that $V_j \sim IG(\eta, h_j^2 \cdot \eta), j=1,\ldots,10$, $\mu=1$ and $\sigma=1$, so that 
...
for(i in 1:N_sim){
  ...
  V_1 <- rbind(V_1, 
               ngme2::rgig(10, 
                           rep(-1,10), 
                           rep(eta + (mu/sigma)^2,10), 
                           eta*h_nig_1d^2 +
                           as.vector((K%*%W_1[i+1,] +h_nig_1d)^2)/sigma^2))
  
  V_2 <- rbind(V_2, ngme2::rgig(10, 
                                rep(-1,10), 
                                rep(eta + (mu/sigma)^2,10), 
                                eta*h_nig_1d^2 +
                                as.vector((K%*%W_2[i+1,] +h_nig_1d)^2)/sigma^2))
}
```
