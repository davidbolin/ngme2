---
title: "Gibbs sampling and prediction for SPDE-based models driven by non-Gaussian noise"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{"Gibbs sampling and prediction for SPDE-based models driven by non-Gaussian noise"}
...
where $\gamma, \mu$ are parameters, $Z\sim N(0,1)$ and is independent of $V$, and $V$ is a positive infinitely divisible random variable. In the SPDE
...
In the above model we will assume $\beta=1$, so we will estimate $\kappa^2$, $\mu$ and
$\sigma_\varepsilon$. 
...
$\mathbf{V}$ is chosen such  that $E[V_i] = h_i$ to ensure parameter identifiability.
...
$$\mathbf{w}|\mathbf{V} \sim N(\mathbf{K}^{-1}(-\mu \mathbf{h}+\mu\mathbf{V}), \mathbf{K}^{-1}{\rm diag}(\mathbf{V})\mathbf{K}^{-1}).$$
Let
$$\mathbf{m} = \mathbf{K}^{-1}(-\mu \mathbf{h}+\mu\mathbf{V})\quad \hbox{and}\quad \mathbf{Q} = \mathbf{K}{\rm diag}(\mathbf{V})^{-1}\mathbf{K}.$$
...
$$V_j|\mathbf{w},\mathbf{Y} \sim GIG(p-0.5, a+\mu^2, b + (\mathbf{K}\mathbf{w}+\mathbf{h}\mu)_j^2).$$
...
For this first example we will consider the latent process $X$ solving the equation
...
We will take $\kappa=1$, $\sigma=1$, $\mu = 1$, $\mathcal{D}=[0,1]$ and $\eta=0.5$. We will also assume that we
have 10 observations
$$Y_i = X(t_i) + \varepsilon_i,\quad i=1,\ldots,10,$$
where $\varepsilon_i \sim N(0,1)$ and $t_1,\ldots,t_{10}\in [0,1]$.
...
V = ngme2::rig(n, a = rep(eta,n), b = h_nig_1d^2*eta, sample.int(10^6, 1))
temp = rnorm(n, mean = -h_nig_1d + V, sd = sqrt(V))
W = solve(K, temp)
...
Y = A_matrix %*% W + rnorm(9)

plot(new_loc_1d, Y, type="h")
abline(0,0)
```

Now, let us assume we want to obtain prediction for $X$ at the point $t^\ast = 5/9$. 
...
V_1 <- matrix(ngme2::rig(10, rep(eta,10), eta*h_nig_1d^2), nrow=1)
V_2 <- matrix(ngme2::rig(10, rep(eta,10), eta*h_nig_1d^2), nrow=1)
```

Recall that $V_j \sim IG(\eta, h_j^2 \cdot \eta), j=1,\ldots,10$, $\mu=1$, so that 
$$V_j|\mathbf{w},\mathbf{Y} \sim GIG(-1,\eta + 1, \eta\cdot h_j^2 + (\mathbf{K}\mathbf{w}+\mathbf{h})_j^2)$$
and
$$\mathbf{w} | \mathbf{V}, \mathbf{Y} \sim N\big( (\mathbf{K}{\rm diag}(\mathbf{V})^{-1}\mathbf{K} + \mathbf{A}^\top\mathbf{A})^{-1}(\mathbf{K}{\rm diag}(\mathbf{V})^{-1}\mathbf{K}(\mu\mathbf{V}-\mathbf{h}\mu)+\mathbf{A}^\top \mathbf{Y}), (\mathbf{K}{\rm diag}(\mathbf{V})^{-1}\mathbf{K} + \mathbf{A}^\top\mathbf{A})^{-1}\big).$$
...
# Gibbs sampling
for(i in 1:N_sim){
  Q_1 <- K%*%diag(1/V_1[i,])%*%K
  Q_2 <- K%*%diag(1/V_2[i,])%*%K
  
  resp_1 <- Q_1%*%solve(K,-h_nig_1d + V_1[i,]) + t(A_matrix)%*%Y/sigma_eps^2
  resp_2 <- Q_2%*%solve(K,-h_nig_1d + V_2[i,]) + t(A_matrix)%*%Y/sigma_eps^2
  ...
  V_1 <- rbind(V_1, 
               ngme2::rgig(10, 
                           ...
                           rep(eta + 1,10), 
                           eta*h_nig_1d^2 +
                           as.vector((K%*%W_1[i+1,] +h_nig_1d)^2)))
  
  V_2 <- rbind(V_2, ngme2::rgig(10, 
                                rep(-1,10), 
                                rep(eta + 1,10), 
                                eta*h_nig_1d^2 +
                                as.vector((K%*%W_2[i+1,] +h_nig_1d)^2)))
}
```
...
A_pred <- inla.spde.make.A(mesh_loc_nig_1d, loc = 5/9)
